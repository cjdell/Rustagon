...
## On-Device Debug Workflow (serial + WebSocket screen scraping)

When a real badge is on the bench, you can debug it **purely agentically**: read
its screen, drive its buttons, and read its serial logs — no human interaction
needed. Full details: **`debug-workflow.md`** (repo root, verified live against
a badge at `192.168.49.144` / `/dev/cu.usbmodem1101`).

**Prerequisites — run everything inside the Nix flake.** The debug tools need
`deno` and `espflash`, both provisioned by `flake.nix` (no system deps; the
tools themselves are dependency-free Deno scripts). Either enter the dev shell
(`nix develop` / direnv) or prefix commands with
`nix develop --command bash -c "..."`.

**The two channels:**

1. **USB serial** (`/dev/cu.usbmodem1101` on macOS, `/dev/ttyACM0` on Linux):
   boot log, panics, WiFi/IP/mDNS status, WASM start/stop. Connect with
   `espflash monitor --port <port> --non-interactive` (USB-Serial/JTAG, so baud
   is irrelevant). Log level is **INFO** by default (`ESP_LOG` is a *build-time*
   env var in `firmware/.cargo/config.toml`; use `ESP_LOG=DEBUG` for per-request
   HTTP logs).
2. **WebSocket** `ws://<ip>/api/ws` (subprotocol `messages`) — the same channel
   the WebUI `/remote` page uses:
   - **Screen:** one binary frame every 250 ms; exactly `240*240/8 = 7200`
     bytes, 1 bit per pixel (LSB-first, bit set = lit). Decode reference:
     `web/src/lib/device/badge.ts`, sender: `app/src/http/web_socket.rs`.
   - **Input:** JSON text `{"HexButton":"Down"}` / `{"HexButton":"DownReleased"}`
     (`Up Down Left Right Fire`, `HexA..HexF`, `Touch01..Touch12`, each with a
     `*Released` twin) and `{"SystemMessage":"BootButton"}`. Injected by
     `websocket_input_forwarder_task` (`firmware/src/bin/rustagon.rs`) into the
     same platform input queues as physical buttons — indistinguishable from
     real presses.

**Tools** — `tools/debug/` (Deno CLI, zero deps). Run via `just` recipes (all
arguments pass straight through; device defaults to `$BADGE_HOST`, then
`192.168.49.144`):

```sh
just debug_shot    [host] [prefix]            # screen -> <prefix>.png/.txt/.frame + hash
just debug_ocr     [host | saved-prefix]      # OCR menu text (10x20) / app text (5x7)
just debug_input   Down Fire                  # button presses (press+release); "boot" = BootButton
just debug_watch   --timeout 6                # exit 0 when the screen hash changes
just debug_record  --dir rec --timeout 8      # record every changed frame
just debug_logs    /dev/cu.usbmodem1101 --out serial.log   # timestamped espflash monitor log
just debug_reboot                             # POST /api/reboot (WS reconnects after)
```

**The loop:**

1. Sanity: `curl -s http://192.168.49.144/api/config` (also `ls /dev/cu.usbmodem*`).
2. Start `just debug_logs ...` in the background (hard-resets the badge → clean
   boot log).
3. `just debug_shot` + `just debug_ocr` to read the current screen; the OCR
   output is your assertion target (`INV` marks the selected menu item).
4. Drive it: `just debug_input Down Fire`, then `just debug_watch --timeout 8`
   and re-OCR. Each press is one menu step (the menu ignores releases).
5. Correlate with the serial log: WASM starts → `Starting WASM on SECOND
   CORE...`; panics → `panicked`/`Backtrace`; WiFi issues → `WiFi Status: ...`.
6. Recover: `just debug_reboot` (soft) or restart `debug_logs` (hard reset).

Notes: the root menu in the current build is just the 10 built-in apps +
`Power Off` (`additional_apps` is `&[]` in `firmware/src/tasks/menu/mod.rs`);
WASM apps are not listed. The menu's selected row is black text on a blinking
gray bar that reads as white in the 1-bit stream (menu geometry:
`MARGIN=40`, `ICON_WIDTH=20`, `CHAR_WIDTH=10`, `LINE_HEIGHT=20` in
`libs/display_renderer/src/lib.rs`). OCR font tables are generated by
`tools/debug/scripts/gen_fonts.py` (pure-stdlib Python, no PIL).
