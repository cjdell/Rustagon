...
impl<P: Platform> MenuApp<P> for PowerInfoApp<P> {
  ...
  async fn run(&mut self, ctx: AppRunContext<'_, P>) -> AppAction {
    // Refresh on (re-)entry — subsumes the old `init`.
    ...
    loop {
      ...
      if let AppRunEvent::Input(AppInput::Button(HexButton::Fire)) = event {
...
async fn build_status_screen<P: Platform>(ctx: &MenuAppContext<P>) -> LcdScreen {
  ...
  lines.push(MenuLine(
    Icon20::Info,
    format!("  {}.{:02}V", status.vbat_mv / 1000, (status.vbat_mv % 1000) / 10),
  ));
  ...
  if status.is_power_present {
    ...
    lines.push(MenuLine(
      Icon20::Info,
      format!("VBUS: {}.{:02}V", status.vbus_mv / 1000, (status.vbus_mv % 1000) / 10),
    ));
  ...
  lines.push(MenuLine(
    Icon20::Info,
    format!("VSYS: {}.{:02}V", status.vsys_mv / 1000, (status.vsys_mv % 1000) / 10),
  ));
  lines.push(MenuLine(
    Icon20::Info,
    format!(
      "VREG: {}.{:02}V",
      status.charge_voltage_mv / 1000,
      (status.charge_voltage_mv % 1000) / 10
    ),
  ));
