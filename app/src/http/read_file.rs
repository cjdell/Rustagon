...
impl RequestHandlerService<()> for ReadFileHandler {
  async fn call_request_handler_service<R: Read, W: picoserve::response::ResponseWriter<Error = R::Error>>(
    ...
    let file_size = match self.storage.get_file_size(file_name.clone()).await {
...
impl Chunks for FileChunks {
  ...
  async fn write_chunks<W: picoserve::io::Write>(
    ...
    loop {
      let buffer = match self.storage.read_binary_chunk(self.file_name.clone(), read_bytes, CHUNK_SIZE).await {
        Ok(buffer) => buffer,
        Err(err) => {
