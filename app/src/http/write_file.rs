...
impl RequestHandlerService<()> for WriteFileHandler {
  async fn call_request_handler_service<R: Read, W: picoserve::response::ResponseWriter<Error = R::Error>>(
    ...
    let mut buffer = external_vec(CHUNK_SIZE);
    ...
    loop {
      ...
      if let Err(err) = self
        .storage
        .write_binary_chunk(file_name.clone(), written_bytes as u32, buffer[..chunk_bytes].to_vec(), last_chunk)
        .await
      {
        self.sender.send(HttpStatusMessage::Idle).await;
      ...
      self
        .sender
        .send(HttpStatusMessage::Progress(written_bytes as u32, file_size as u32))
        .await;
