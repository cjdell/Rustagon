...
//! reading. Our driver just maps the key events to `DeviceEvent::Keyboard`
//! and injects navigation keys into the menu system.
...
fn key_number_to_keycode(key: u8) -> Option<KeyCode> {
  match key {
    ...
    53 => Some(KeyCode::Right), 54 => Some(KeyCode::Slash),
    55 => Some(KeyCode::Up),
