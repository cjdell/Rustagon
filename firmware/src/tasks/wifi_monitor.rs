...
pub async fn wifi_monitor_task(
  ...
  loop {
    // Wait for status change (uses WatchedValue internally)
    let status = platform.wifi_manager().wait_for_status_change().await;
    println!("WiFi Status: {:?}", status);

    match status {
      WifiStatus::Connected(ipv4_addr) => {
        lcd_signal.signal(LcdScreen::Headline(Icon40::Wifi, String::from("Connected")));
        ...
        lcd_signal.signal(LcdScreen::Headline(Icon40::Info, format!("IP: {}", ipv4_addr)));
        Timer::after(Duration::from_millis(2_000)).await;
      }
      WifiStatus::AccessPoint => {
        lcd_signal.signal(LcdScreen::Headline(Icon40::Info, String::from("AP Mode Active")));
        let _ = platform.led_manager().request(LedRequest::Fire);
        Timer::after(Duration::from_millis(2_000)).await;
        lcd_signal.signal(LcdScreen::Headline(
          Icon40::Info,
          format!("AP: {}", device_state.get_data().ap_ssid),
        ));
        Timer::after(Duration::from_millis(2_000)).await;
        lcd_signal.signal(LcdScreen::Headline(Icon40::Info, String::from("IP: 192.168.1.1")));
        Timer::after(Duration::from_millis(2_000)).await;
      }
      WifiStatus::Offline | WifiStatus::NoNetworksFound | WifiStatus::Interrupted => {
        // Connection lost or not attempting to connect
        lcd_signal.signal(LcdScreen::Headline(Icon40::Warn, String::from("WiFi Offline")));
        let _ = platform.led_manager().request(LedRequest::Solid(LedState { r: 255, g: 0, b: 0 }));
        Timer::after(Duration::from_millis(1_000)).await;
      }
      WifiStatus::Connecting => {
        // Attempting to connect
        lcd_signal.signal(LcdScreen::Headline(Icon40::Info, String::from("Connecting...")));
