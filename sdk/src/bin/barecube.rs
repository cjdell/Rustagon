// A blocking version of the cube sample (no yield)
...
use crate::{
  ...
  protocol::{extern_get_millis, extern_set_lcd_buffer},
};
...
fn wasm_main() {
  ...
  let start = unsafe { extern_get_millis() };
  ...
  loop {
    let now = unsafe { extern_get_millis() };
    ...
    let rotation_speed = 2.0 * PI / (ANIMATION_DURATION as f32); // Full rotation in 10 seconds
